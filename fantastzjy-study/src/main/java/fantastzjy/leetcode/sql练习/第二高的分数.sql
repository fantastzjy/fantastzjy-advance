...
：
            ...
            offset n表示跳过x条语句
            limit y offset x 分句表示查询结果跳过 x 条数据
，读取前 y 条数据
            使用limit和offset
，降序排列再返回第二条记录可以得到第二大的值
。

select distinct 成绩
from 成绩表
where 课程 = '语文'
order by 课程, 成绩 desc limit 1,1;
...
select NULL
       ...
       查询的结果为 {"headers": ["SecondHighestSalary"], "values": []}
       而实际上预期结果为
       {"headers": ["SecondHighestSalary"], "values": [[null]]}

    2、为什么select后面可以不接from单独用
       因为  子查询可以在任意地方使用。相当于查询的列。在oracle中只写查询列没有查询表需要加上
from dual（伪表）。而mysql不用写
    3、LIMIT 子句 限制结果数量
