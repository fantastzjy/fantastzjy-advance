 ...
 */
public class HeapSort_左神_size改为rightBound {
    ...
    public static void heapSort(int[] arr) {
        ...
        int rightBound = arr.length - 1;//堆的有效size
        //从小到大排序
        //上面插入完之后已经是一个有序堆了  可以直接进行交换
        swap(arr, 0, rightBound);//最大值，和堆中最后一个数交换，堆有效size-1
        //nlogn
        while (rightBound > 0) {//O(N)次
            //树化    从0 开始  长度为size  size不是数组最后一个位置
            heapify(arr, 0, --rightBound);//O(logN)   当size=1时  下面是和本身进行交换
            swap(arr, 0, rightBound);
        }
    }
    ...
    public static void heapify(int[] arr, int index, int rightBound) {
        int left = index * 2 + 1;//左孩子的位置
        //如果当前位置的数有孩子的话，循环发生    如果没有的话 就是叶子结点了
        while (left <= rightBound) {         //要是把size改为rightbound 这啦也要改成<=
            //两个孩子最大值的坐标
            //如果右孩子有的话，和左孩子的值比较，最大的值的下标，作为largest的值
            int largest = left + 1 <= rightBound //右孩子如果不越界
