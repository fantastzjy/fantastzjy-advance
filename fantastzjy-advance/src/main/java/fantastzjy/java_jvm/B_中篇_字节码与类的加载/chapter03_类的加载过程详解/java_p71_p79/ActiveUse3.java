package fantastzjy.java_jvm.B_中篇_字节码与类的加载.chapter03_类的加载过程详解.java_p71_p79;
 ...
 * 7. 当虚拟机启动时，用户需要指定一个要执行的主类（包含main()方法的那个类），虚拟机会先初始化这个主类。《和调用静态方法其实是一样的，main也是静态方法》
 ...
 * >在初始化一个类时，并不会先初始化它所实现的接口  //查看堆栈信息时确实可以看到加载过了接口类，说明执行了加载，没有初始化
